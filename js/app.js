(() => {
    ...
    const initMobileMenu = () => {
        const toggle = () => {
            ...
            if (!mobileMenuNav.classList.contains("mobile-menu__nav--active")) mobileMenuNav.classList.add("mobile-menu__nav--active"); else mobileMenuNav.classList.remove("mobile-menu__nav--active");
        };
    ...
    const _mobile_menu = initMobileMenu;
    ...
    const coming_soon = initComingSoonExpand;
    class ConnectForm {
        ...
        initNameField() {
            this.storage.userData.name = {
                data: null,
                correct: false
            };
            const regex = /^[a-zA-Z]{2,}\s[a-zA-Z]{2,}$/;
            const inputType = "name";
            const nameInput = this.form.querySelector(`.${this.formClassName}__input--name`);
            const workingInput = new ConnectFormInput(nameInput, regex, inputType, this.storage);
            workingInput.init();
        }
        initEmailField() {
            this.storage.userData.email = {
                data: null,
                correct: false
            };
            const regex = /^[a-z0-9]+@gmail\.com$/;
            const inputType = "email";
            const emailInput = this.form.querySelector(`.${this.formClassName}__input--email`);
            const workingInput = new ConnectFormInput(emailInput, regex, inputType, this.storage);
            workingInput.init();
        }
        initAboutField() {
            this.storage.userData.about = {
                data: null,
                correct: false
            };
            const regex = /^[a-zA-Z0-9\s\-\'\!\?\.\,]{10,500}$/;
            const inputType = "about";
            const aboutInput = this.form.querySelector(`.${this.formClassName}__input--about`);
            const workingInput = new ConnectFormInput(aboutInput, regex, inputType, this.storage);
            workingInput.init();
        }
        initSubmitButton() {
        ...
        submit(e) {
            e.preventDefault();
            let check = true;
            for (const key in this.userData) if (false === this.storage.userData[key].correct) check = false;
            if (!check) alert("Unable to submit the form"); else this.finish();
        }
        finish() {
            const form = this.form.querySelector(`.${this.formClassName}__form`);
            form.classList.add(`${this.formClassName}__form--hidden`);
            const successMsg = this.form.querySelector(`.${this.formClassName}__success`);
    ...
    class ConnectFormInput {
        ...
        check() {
            ...
            if (userInput.match(this.regex)) this.isCorrect(true); else this.isCorrect(false);
        }
        isCorrect(correct) {
            this.storage.userData[this.type]["data"] = this.input.value;
            if (!correct) this.storage.userData[this.type]["correct"] = false; else if (correct) this.storage.userData[this.type]["correct"] = true;
        }
    }
    ...
    const _connect_form = initConnectForm;
    class SubscribeForm {
        constructor(form, className) {
            this.form = form;
        ...
        checkData(e) {
            ...
            if (userInput.match(regex)) this.submitForm(true); else this.submitForm(false);
        }
        submitForm(isDataCorrect) {
            const form = this.form.querySelector(`.${this.className}__form`);
            const successMessage = this.form.querySelector(`.${this.className}__success`);
            if (isDataCorrect) {
    ...
    const _subscribe_form = SubscribeForm;
    const initSubscribeForms = () => {
        const className = "subscribe-form";
        const forms = document.querySelectorAll(`.${className}`);
        if (!forms) return;
        forms.forEach((form => {
            const newForm = new _subscribe_form(form, className);
            newForm.init();
        }));
    };
    const subscribe_forms = initSubscribeForms;
